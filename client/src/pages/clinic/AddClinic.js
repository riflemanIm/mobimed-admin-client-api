...
const AddClinic = () => {
  ...
  console.log("activeStep", activeStep, "clinic_id", clinic_id);
  //  let clinic_id = null;
  //  if (currentClinic?.clinic_id) clinic_id = currentClinic.clinic_id;
  ...
  const deleteOneImage = async () => {
    if (clinic_id) {
      ...
      await deleteAvararServer(
        `${config.baseURLimages}/clinics/upload-avatar/${clinic_id}`
      )
        .then((res) => {
  ...
  const handleFile = async (event) => {
    ...
    if (
      ...
      const filename = `${clinic_id}.${extension}`;

      setIsLoadingImg(true);
      await uploadToServer(
        `${config.baseURLApi}/clinics/upload-avatar/${clinic_id}`,
        { filedata, filename }
      )
        .then((res) => {
          setIsLoadingImg(false);
  ...
  function sendNotification(errorMessage = null) {
    const componentProps = {
      type: "feedback",
      message: errorMessage != null ? errorMessage : "Clinic added!",
      variant: "contained",
  ...
  const handleNext = () => {
    ...
    if (activeStep === 0 && values.clinicname !== "") {
      actions.doCreate(
        { clinicname: values.clinicname, password: md5(values.password) },
        handleBack,
        history
      )(managementDispatch, sendNotification);
    }
    if (activeStep === 1 && clinic_id != null) {
      actions.doUpdate(
        clinic_id,
        values,
        history
              ...
              {activeStep === 0 ? (
                <>
                  <TextField
                    ...
                    placeholder="Clinicname или Email "
                    type="text"
                    fullWidth
                    required
                    ...
                    <div className={classes.imgWrap}>
                      {isLoadingImg ? (
                        ...
                        <>
                          ...
                          <img
                            src={`${config.baseURLApi}/clinics/photo/${clinic_id}`}
                            alt=""
                            height={"100%"}
                          />
                        </>
